...
class MinesweeperAI():
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                if (i, j) != cell and 0 <= i < self.height and 0 <= j < self.width:
                    neighbers.add((i, j))
        neighbers = neighbers - self.safes
        for mine in neighbers:
            if mine in self.mines:
                cnt_mine += 1
        neighbers = neighbers - self.mines
        self.knowledge.append(Sentence(neighbers, count - cnt_mine))
        while True: 
    ...
    def conclude(self):
        """
        Checks the knowledge base for any sentences that are
        known to be safe or mines, and updates the knowledge base
        accordingly. Returns the number of new sentences added.
        """
        # new_sentences = 0
        # for sentence in self.knowledge:
    ...
    def infer(self):
        """
        Checks the knowledge base for any sentences that can be inferred
        from other sentences, and updates the knowledge base accordingly.
